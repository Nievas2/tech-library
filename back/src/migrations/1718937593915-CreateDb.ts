...
export class CreateDb1718937593915 implements MigrationInterface {
    name = 'CreateDb1718937593915'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`users\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`username\` varchar(50) NOT NULL, \`password\` varchar(255) NOT NULL, \`email\` varchar(50) NOT NULL, \`role\` enum ('USER', 'ADMIN', 'CUSTOMER') NOT NULL DEFAULT 'USER', \`is_active\` tinyint NOT NULL DEFAULT 1, UNIQUE INDEX \`IDX_772886e2f1f47b9ceb04a06e20\` (\`username\`, \`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`librarys\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`name\` varchar(25) NOT NULL, \`description\` varchar(255) NOT NULL, \`link\` varchar(300) NOT NULL, \`is_active\` tinyint NOT NULL DEFAULT 1, \`state\` enum ('ACTIVE', 'PENDING', 'INACTIVE') NOT NULL DEFAULT 'PENDING', \`created_by_id\` bigint NOT NULL, UNIQUE INDEX \`IDX_1aa54403e37480de0c222c9a2f\` (\`name\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
