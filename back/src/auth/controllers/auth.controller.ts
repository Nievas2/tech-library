...
import { ResponseToken } from "../utils/token.response.interface";
...
export class AuthController {
  constructor(
    ...
    private readonly authHttpResponse: AuthHttpResponse = new AuthHttpResponse(
      authService.getEnvironment("LENGUAGE")
    ),
  ...
  /**
   * @method loginGitHub
   * @description Método que se encarga de manejar las peticiones de login con GitHub y redirigir al front los datos
   * @param req
   * @param res
   * @returns status 200 con {accessToken: token, user: user}
   * @returns status 500 si hay un error en el servidor
